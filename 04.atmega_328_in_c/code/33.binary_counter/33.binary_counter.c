...
#define F_CPU 8000000UL    // I'm using the ATmega328P chip without an external crystal oscillator.
...
int main(void) {
  ...
  int pause_trigger = 1;    // Intentionally set so that counter is on standby when powered.
  int current_value = 0;
  ...
  while (1) {
    ...
    if (!pause_trigger) {
      PORTB = current_value;
      _delay_ms(70);
      current_value++;  // It automatically resets at 0xff + 0x01 = 0x00, because the 9th bit is discarded.
    } else if (pause_trigger) {
      PORTB = current_value;
    } 
  }       
  return 0;   // Program execution will never get to this point. I only wrote this line because I used int main().
}
